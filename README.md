...
工時管家是以 **BeeWare Toga + SQLite** 製作的繁體中文、離線、單人手機工時管理 App。所有時數在核心與資料庫均以整數分鐘運算；不登入、不連線、不蒐集資料。本版版本為 `0.2.0`。
...
`python build_mobile.py android` 採 incremental build：找不到 Android Gradle scaffold 時才執行一次 `briefcase create android`，後續改用 `briefcase update android`。需要刻意刪除 Android scaffold/cache 並重建時，使用 `python build_mobile.py android --clean`。APK 一律透過 `briefcase package android -p apk` 產生，再遞迴尋找新 APK，複製為 `release/android/工時管家-0.2.0-release.apk` 或 `工時管家-0.2.0-debug.apk`；即時輸出也會保存於 `release/build_android.log`。
