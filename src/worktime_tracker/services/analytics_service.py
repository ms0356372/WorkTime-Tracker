...
def summarize(records: list[WorkRecord]) -> Summary:
    ...
    standard = sum(r.standard_minutes for r, _ in values)
    longest = max(values, key=lambda x: x[1], default=None)
    differences = [value - record.standard_minutes for record, value in values]
    return Summary(
        total,
        standard,
        total - standard,
        len(values),
        round(mean([v for _, v in values])) if values else 0,
...
def calculate_month_summary(
    records: list[WorkRecord], year: int, month: int
) -> Summary:
    return summarize(
        [r for r in records if r.work_date.year == year and r.work_date.month == month]
    )
