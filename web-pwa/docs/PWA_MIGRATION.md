...
5. 月分析只選該 calendar month。年分析以 UI 所選月份的 `year`，不可固定 today.year。平均為總分鐘除出勤日的整數結果；假日/休息日實作依 calendar standard=0，因此工作分鐘列假日工作與 overtime。
...
- [ ] missing workday、official holiday fetch/update、special-date UI
- [ ] monthly/year analysis with selected year and ledger balances
