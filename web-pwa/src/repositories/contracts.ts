...
export interface WorkRecordRepository { save(record:WorkRecord):Promise<number>; getByDate(date:string):Promise<WorkRecord|undefined>; recordsForMonth(year:number,month:number):Promise<WorkRecord[]>; recordsForYear(year:number):Promise<WorkRecord[]>; recent(limit?:number):Promise<WorkRecord[]>; delete(id:number):Promise<void> }
