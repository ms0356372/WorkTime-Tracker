...
export type SpecialDateCategory = 'COMPANY_MAKEUP_WORKDAY'|'COMPANY_HOLIDAY'|'SPECIAL_NON_WORKDAY'
export type TransactionType = 'WORKTIME_EARN'|'WORKTIME_DEDUCTION'|'MONTHLY_SETTLEMENT'|'LEAVE_CONVERSION'|'REVERSAL'|'ADJUSTMENT'|'MISSING_WORKDAY_DEDUCTION'|'ANNUAL_LEAVE_GRANT'|'ANNUAL_LEAVE_SETTLEMENT'|'COMP_LEAVE_SETTLEMENT'|'COMP_MONTHLY_TRANSFER'|'COMP_MONTHLY_CASH_SETTLEMENT'|'COMP_DAILY_EXCESS_ACCRUAL'|'COMP_ANNUAL_EXCESS_CASH_SETTLEMENT'
...
export interface CalendarOverride { id?:number; workDate:ISODate; dayType:CalendarDayType; category?:SpecialDateCategory; note:string; createdAt:string; updatedAt:string }
export interface OfficialHoliday { holidayDate:ISODate; name:string; year:number; source:string; syncedAt:string }
export interface CompSettlementPolicy { id?:number; effectiveFrom:ISODate; mode:'ANNUAL'|'MONTHLY'; monthlyCapMinutes:number; cashHourlyRateCents:number; dailyCapEnabled:boolean; dailyCapMinutes:number; createdAt:string }
