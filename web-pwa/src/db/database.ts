...
export interface LegacyMonthlySettlement { id:number; year:number; month:number; minutes:number; rule:string }
...
export class WorkTimeDatabase extends Dexie {
  ...
  legacyMonthlySettlements!:EntityTable<LegacyMonthlySettlement,'id'>
  constructor(databaseName = 'worktime-tracker-pwa'){super(databaseName);this.version(1).stores({
    ...
    workRecords:'++id,&workDate',settings:'&key,effectiveDate',ledger:'++id,entryDate,transactionDatetime,transactionType,ledgerOrigin,sourceRecordId,reversalOfId',leaveCycles:'++id,&[startDate+endDate]',compLeaveCycles:'++id,&[startDate+endDate]',calendarOverrides:'++id,&workDate,category',officialHolidays:'&holidayDate,year',compPolicies:'++id,&effectiveFrom',compMonthlySettlements:'++id,&[year+month]',appMetadata:'&key',legacyMonthlySettlements:'&id,&[year+month]'
  })}
}
