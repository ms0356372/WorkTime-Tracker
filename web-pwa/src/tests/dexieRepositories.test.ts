...
describe('ledger deterministic persistence',()=>{
 it('atomically replaces system rows while retaining manual payload',async()=>{const repository=new DexieLedgerRepository(createDatabase()),system=ledgerEntry(),manual={...ledgerEntry(),ledgerOrigin:'MANUAL' as const,reason:'manual audit',transactionDatetime:'2026-09-04T08:00:00'};await repository.add(system);const manualId=await repository.add(manual);await repository.replaceSystem([{...manual,id:manualId,compBalance:30},{...system,compChange:60,compBalance:90}]);expect(await repository.getSystem()).toHaveLength(1);expect(await repository.getManual()).toMatchObject([{id:manualId,reason:'manual audit',compChange:30}]);expect(await repository.currentBalances()).toEqual({compBalanceMinutes:90,annualBalanceMinutes:0})})
 it('retains persisted ledger across repository instances and supports ranges',async()=>{const database=createDatabase(),first=new DexieLedgerRepository(database);await first.add(ledgerEntry());const second=new DexieLedgerRepository(database);expect(await second.all()).toHaveLength(1);expect(await second.entriesForRange('2026-09-01','2026-09-30')).toHaveLength(1)})
})
